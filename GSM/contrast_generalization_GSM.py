...
def main():
    ...
    if x_noise:
        subfolder = "generalization/w_noise/"
    else:
        subfolder = "generalization/no_noise/"
    ...
    input_scaling = np.loadtxt(in_location+"/learnt_parameters/input_scaling_learn")
    input_baseline = np.loadtxt(in_location+"/learnt_parameters/input_baseline_learn")
    input_pow = np.loadtxt(in_location+"/learnt_parameters/input_nl_pow_learn")
    ...
    for alpha in range(n_targets):
        ...
        #mat_plot(np.reshape(x_array[alpha],(L,L)),"x"+str(alpha),scale,plot_location)
    ...
    for alpha in range(n_targets):
        ...
        h_array[alpha] = get_true_h_from_x_proj(np.dot(A.T,x_array[alpha]),h_scale,input_scaling,input_baseline,input_pow)
    ...
    for alpha in range(n_targets):
        ...
        np.savetxt(targets_location+"/mu"+str(alpha),baseline + mu_post_array_true_z[alpha])
        np.savetxt(targets_location+"/sigma"+str(alpha),Sigma_post_array_true_z[alpha])
        np.savetxt(targets_location+"/std"+str(alpha),std_post_array_true_z[alpha])
    
    
    #multimat_plot(Sigma_post_array_true_z,n_targets,"Posterior covariances true z",plot_location)
    #multimat_plot(Sigma_post_array_z_MAP,n_targets,"Posterior covariances z MAP",plot_location)
    #multimat_plot(Sigma_post_array_full_inf,n_targets,"Posterior covariances full inf",plot_location)
