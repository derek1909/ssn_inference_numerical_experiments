...
def main():
    ...
    if x_noise:
        subfolder = "generalization_data/w_noise/"
    else:
        subfolder = "generalization_data/no_noise/"
        
    results_location = subfolder+"results_GSM"
    targets_location = subfolder+"targets_GSM"
    
    in_location = "../SSN/parameter_files"
    ...
    s_x_2 = s_x**2
    k = 2                   # Shape parameter of the gamma dist. for z
    theta = 0.25               # Scale parameter of the gamma dist. for z
    ...
    x_array = np.empty([N_points,D_x])
    
    h_array = np.empty([N_points,2*D_y])
    ...
    z_MAP_array = np.empty([N_points])
    
    z_array = np.random.gamma(k, theta, N_points)
    
    original_bump = np.loadtxt(in_location+"/y_0")
    mean_array = np.full((D_y),np.average(original_bump))
    
    y_array = np.random.multivariate_normal(mean = mean_array, cov = C, size = N_points) 
    
    for alpha in range (N_points): 
        print("point "+str(alpha))
        ...
        mu_post_array_true_z,Sigma_post_array_true_z = get_post_moments(
                                                    x_array[alpha],z_array[alpha],s_x_2,A,ATA,C_inv)
        std_post_array_true_z = np.sqrt(np.diag(Sigma_post_array_true_z))
        
        # Moments at MAP contrast
        z_MAP_array[alpha] = p_z[0][np.argmax(p_z[1])]
        
        mu_post_array_z_MAP,Sigma_post_array_z_MAP = get_post_moments(
                                                x_array[alpha],z_MAP_array[alpha],s_x_2,A,ATA,C_inv)
        std_post_array_z_MAP = np.sqrt(np.diag(Sigma_post_array_z_MAP))
        
        # Full inference 
        mu_post_array_full_inf,Sigma_post_array_full_inf = get_post_moments_full_inference(x_array[alpha],p_z,s_x_2,A,ATA,C_inv)
        
        std_post_array_full_inf = np.sqrt(np.diag(Sigma_post_array_full_inf))
        
        np.savetxt(results_location+"/z_"+str(alpha),np.expand_dims(z_array[alpha], axis = 0))
        np.savetxt(results_location+"/x_"+str(alpha),x_array[alpha])
        np.savetxt(results_location+"/x_sqr"+str(alpha),x_array[alpha].reshape((L,L)))
        np.savetxt(results_location+"/y_"+str(alpha),y_array[alpha])
        np.savetxt(results_location+"/h_true_"+str(alpha),h_array[alpha])
        
        np.savetxt(results_location+"/mu_true_z_"+str(alpha),mu_post_array_true_z)
        np.savetxt(results_location+"/Sigma_true_z_"+str(alpha),Sigma_post_array_true_z)
        np.savetxt(results_location+"/std_true_z_"+str(alpha),std_post_array_true_z)
        
        np.savetxt(results_location+"/mu_z_MAP_"+str(alpha),mu_post_array_z_MAP)
        np.savetxt(results_location+"/Sigma_z_MAP_"+str(alpha),Sigma_post_array_z_MAP)
        np.savetxt(results_location+"/std_z_MAP_"+str(alpha),std_post_array_z_MAP)
        
        np.savetxt(results_location+"/mu_full_inf_"+str(alpha),mu_post_array_full_inf)
        np.savetxt(results_location+"/Sigma_full_inf_"+str(alpha),Sigma_post_array_full_inf)
        np.savetxt(results_location+"/std_full_inf_"+str(alpha),std_post_array_full_inf)
        
        np.savetxt(targets_location+"/x_"+str(alpha),x_array[alpha])
        np.savetxt(targets_location+"/y_"+str(alpha),y_array[alpha])
        np.savetxt(targets_location+"/h"+str(alpha),h_array[alpha])        
        np.savetxt(targets_location+"/mu"+str(alpha),baseline + mu_post_array_true_z)
        np.savetxt(targets_location+"/sigma"+str(alpha),Sigma_post_array_true_z)
        np.savetxt(targets_location+"/std"+str(alpha),std_post_array_true_z)
